...
def test_class_i2v(base_model_type):
    # Frames2Video is treated as an I2V-style model
    return base_model_type in [
        "i2v",
        "i2v_2_2",
        "fun_inp_1.3B",
...
def test_oneframe_overlap(base_model_type):
    return (
        test_class_i2v(base_model_type)
        and not (test_multitalk(base_model_type) or base_model_type in ["animate", "scail"])
...
class family_handler:
    ...
    def get_lora_dir(base_model_type, args):
        i2v = test_class_i2v(base_model_type) and base_model_type not in [
            "i2v_2_2",
            "i2v_2_2_multitalk",
        ]
        wan_dir = (
            getattr(args, "lora_dir_wan", None)
            or getattr(args, "lora_dir", None)
            or os.path.join("loras", "wan")
        )
        wan_i2v_dir = (
            getattr(args, "lora_dir_wan_i2v", None)
            or getattr(args, "lora_dir_i2v", None)
            or os.path.join("loras", "wan_i2v")
        )
        wan_1_3b_dir = (
            getattr(args, "lora_dir_wan_1_3b", None)
            or os.path.join("loras", "wan_1.3B")
        )
        wan_5b_dir = (
            getattr(args, "lora_dir_wan_5b", None)
            or os.path.join("loras", "wan_5B")
        )
    ...
    def set_cache_parameters(cache_type, base_model_type, model_def, inputs, skip_steps_cache):
        ...
        if cache_type == "mag":
            ...
            if base_model_type in ["t2v", "mocha"] and "URLs2" in model_def:
                ...
                if (
                    inputs.get("image_start", None) is not None
                    and inputs.get("video_source", None) is not None
                ):  # t2v
                    def_mag_ratios = [
                        0.99505,
                        0.99389,
    ...
    def query_model_def(base_model_type, model_def):
        ...
        extra_model_def["multitalk_class"] = multitalk = test_multitalk(
            base_model_type
        )
        extra_model_def["standin_class"] = standin = test_standin(
            base_model_type
        )
        ...
        # Explicit Frames2Video family flag
        ...
        # Frames2Video: force I2V behavior + morph-related flags
        ...
        if (
            test_class_t2v(base_model_type)
            or vace_class
            or base_model_type in ["chrono_edit"]
        ) and base_model_type not in ["alpha"]:
        ...
        extra_model_def.update(
            {
                ...
                "sliding_window": base_model_type
                in [
                    "multitalk",
                    "infinitetalk",
                    "t2v",
                ...
                or test_class_i2v(base_model_type)
                or test_wan_5B(base_model_type)
                or vace_class,
                ...
                "tea_cache": not (
                    base_model_type in ["i2v_2_2"]
                    or test_wan_5B(base_model_type)
                    or multiple_submodels
                ),
                "mag_cache": True,
                "keep_frames_video_guide_not_supported": base_model_type
                in ["infinitetalk"],
                "sample_solvers": [
                    ("unipc", "unipc"),
                    ("euler", "euler"),
            ...
            if base_model_type in ["i2v_2_2"]:
                ...
                extra_model_def["guide_custom_choices"] = {
                    "choices": [
                        ...
                        (
                            "Video to Video guided by Text Prompt & Image",
                            "GUV",
                        ),
                        (
                            "Video to Video guided by Text/Image Prompt and Restricted to the Area of the Video Mask",
                            "GVA",
                        ),
                    ],
                    "default": "",
        ...
        # Include Frames2Video in black_frame behavior to keep start frame handling aligned
            ...
            if not alpha:
                extra_model_def["guide_custom_choices"] = {
                    "choices": [
                        ...
                        (
                            "Video to Video guided by Text Prompt and Restricted to the Area of the Video Mask",
                            "GVA",
                        ),
                    ],
                    "default": "",
        ...
        if base_model_type in ["steadydancer"]:
            extra_model_def["guide_custom_choices"] = {
                "choices": [
                    (
                        "Use Control Video Poses to Animate Person in Start Image",
                        "V",
                    ),
                    (
                        "Use Control Video Poses filterd with Mask Video to Animate Person in Start Image",
                        "VA",
                    ),
                ],
                "default": "PVB",
            ...
            extra_model_def["custom_preprocessor"] = (
        ...
        if base_model_type in ["infinitetalk"]:
            ...
            extra_model_def[
                "all_image_refs_are_background_ref"
            ] = True
            extra_model_def["guide_custom_choices"] = {
                "choices": [
                    (
                        "Images to Video, each Reference Image will start a new shot with a new Sliding Window",
                        "KI",
                    ),
                    (
                        "Sparse Video to Video, one Image will by extracted from Video for each new Sliding Window",
                        "RUV",
                    ),
                    (
                        "Video to Video, amount of motion transferred depends on Denoising Strength",
                        "GUV",
                    ),
                ],
                "default": "KI",
        ...
        if base_model_type in ["animate"]:
            extra_model_def["guide_custom_choices"] = {
                "choices": [
                    (
                        "Animate Person in Reference Image using Motion of Whole Control Video",
                        "PVBKI",
                    ),
                    (
                        "Animate Person in Reference Image using Motion of Targeted Person in Control Video",
                        "PVBXAKI",
                    ),
                    (
                        "Replace Person in Control Video by Person in Ref Image",
                        "PVBAIH#",
                    ),
                    (
                        "Replace Person in Control Video by Person in Ref Image. See Through Mask",
                        "PVBAI#",
                    ),
                ],
                "default": "PVBKI",
            ...
            extra_model_def[
                "keep_frames_video_guide_not_supported"
            ] = True
            extra_model_def["extract_guide_from_window_start"] = True
            extra_model_def["forced_guide_mask_inputs"] = True
            extra_model_def["no_background_removal"] = True
            ...
            ] = "Remove Backgrounds behind People (Animate Mode Only)"
        ...
        if vace_class:
            ...
            extra_model_def[
                "background_removal_label"
            ] = "Remove Backgrounds behind People / Objects, keep it for Landscape or Positioned Frames"
            extra_model_def["video_guide_outpainting"] = [0, 1]
            ...
            if lynx:
                ...
                extra_model_def["image_ref_choices"]["choices"] = [
                    ("None", ""),
                    (
                        "People / Objects (if any) then a Face",
                        "I",
                    ),
                    (
                        "Landscape followed by People / Objects (if any) then a Face",
                        "KI",
                    ),
                    (
                        "Positioned Frames followed by People / Objects (if any) then a Face",
                        "FI",
                    ),
                ]
                extra_model_def[
                    "background_removal_label"
                ] = "Remove Backgrounds behind People / Objects, keep it for Landscape, Lynx Face or Positioned Frames"
                extra_model_def["no_processing_on_last_images_refs"] = 1

            if base_model_type in ["vace_ditto_14B"]:
                del extra_model_def["guide_preprocessing"]
                del extra_model_def["image_ref_choices"]
                del extra_model_def["video_guide_outpainting"]
                extra_model_def["mask_preprocessing"] = {
                    "selection": ["", "A"],
                }
        ...
        if (not vace_class) and lynx:
            ...
            extra_model_def["guide_custom_choices"] = {
                "choices": [
                    ("Use Reference Image which is a Person Face", ""),
                    (
                        "Video to Video guided by Text Prompt & Reference Image",
                        "GUV",
                    ),
        ...
        if vace_class or base_model_type in [
            "animate",
            "t2v",
            "t2v_2_2",
            "lynx",
            ...
            image_prompt_types_allowed = "TVL"
        elif multitalk or base_model_type in [
            "fantasy",
            "steadydancer",
            "scail",
        ]:
        ...
        if base_model_type in ["mocha"]:
            extra_model_def["guide_custom_choices"] = {
                "choices": [
                    (
                        "Transfer Person In Reference Images (Second Image must be a Close Up) in Control Video",
                        "VAI",
                    ),
                ],
                "default": "VAI",
    ...
    def get_rgb_factors(base_model_type):
        ...
        latent_rgb_factors, latent_rgb_factors_bias = get_rgb_factors(
            "wan", base_model_type
        )
        return latent_rgb_factors, latent_rgb_factors_bias

    @staticmethod
    def query_model_files(
        computeList, base_model_type, model_filename, text_encoder_quantization
    ):
        text_encoder_filename = family_handler.get_wan_text_encoder_filename(
        ...
        download_def = [
            {
                ...
                "fileList": [
                    ...
                    wan_files + computeList(model_filename),
                ],
            }
        ]
    ...
    def custom_preprocess(
        ...
        **kwargs,
        ...
        if base_model_type == "scail":
            extract_max_people = (
                lambda s: int(m.group(1)) if (m := re.search(r"#(\d+)#", s)) else 1
            )
            ...
            processor = ScailPoseProcessor(
                multi_person=scail_multi_person, max_people=scail_max_people
            )
            ...
            video_guide_processed, video_guide_processed2 = (
                outputs["pose_only"],
                outputs["pose_aug"],
            )
            if video_guide_processed.numel() == 0:
    ...
    def fix_settings(base_model_type, settings_version, model_def, ui_defaults):
        ...
        if settings_version < 2.24:
            if (
                model_def.get("multiple_submodels", False)
                or ui_defaults.get("switch_threshold", 0) > 0
            ) and ui_defaults.get("guidance_phases", 0) < 2:
                ui_defaults["guidance_phases"] = 2

        if settings_version == 2.24 and ui_defaults.get(
            "guidance_phases", 0
        ) == 2:
            mult = model_def.get("loras_multipliers", "")
            if len(mult) > 1 and len(mult[0].split(";")) == 3:
                ui_defaults["guidance_phases"] = 3
        ...
        if settings_version < 2.32:
            ...
            if (
                test_class_i2v(base_model_type)
                and len(image_prompt_type) == 0
                and "S" in model_def.get("image_prompt_types_allowed", "")
            ):
        ...
        if settings_version < 2.40:
            if base_model_type in ["animate"]:
                remove_background_images_ref = ui_defaults.get(
                    "remove_background_images_ref", None
                )
    ...
    def update_default_settings(base_model_type, model_def, ui_defaults):
        ...
        if test_class_i2v(base_model_type) and "S" in model_def[
            "image_prompt_types_allowed"
        ]:
            ui_defaults["image_prompt_type"] = "S"
        ...
        if base_model_type in ["i2v_2_2"]:
            ui_defaults.update(
                {"masking_strength": 0.1, "denoising_strength": 0.9}
            )

        if base_model_type in ["chrono_edit"]:
            ui_defaults.update(
                {"image_mode": 1, "prompt_enhancer": "TI"}
            )
    ...
    def validate_generative_settings(base_model_type, model_def, inputs):
        if base_model_type in ["infinitetalk"]:
            ...
            if ("V" in image_prompt_type or "L" in image_prompt_type) and image_refs is None:
                video_prompt_type = (
                    video_prompt_type.replace("I", "").replace("K", "")
                )
            ...
            if image_refs is not None and len(image_refs) == 1 and "K" in video_prompt_type:
                gr.Info(
                    "Warning, Ref Image that contains the Face to transfer is Missing: if 'Landscape and then People or Objects' is selected beside the Landscape Image Ref there should be another Image Ref that contains a Face."
                )
