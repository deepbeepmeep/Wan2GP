...
py_logging.getLogger("fla.utils").setLevel(py_logging.ERROR)
...
_DEFAULT_CHUNK_GATED_DELTA_RULE = chunk_gated_delta_rule
_DEFAULT_FUSED_RECURRENT_GATED_DELTA_RULE = fused_recurrent_gated_delta_rule
_DEFAULT_FUSED_RMSNORM_GATED = FusedRMSNormGated


def configure_qwen35_vl_safe_legacy_kernels(enabled: bool) -> None:
    global chunk_gated_delta_rule, fused_recurrent_gated_delta_rule, FusedRMSNormGated
    if enabled:
        chunk_gated_delta_rule = None
        fused_recurrent_gated_delta_rule = None
        FusedRMSNormGated = None
        return
    chunk_gated_delta_rule = _DEFAULT_CHUNK_GATED_DELTA_RULE
    fused_recurrent_gated_delta_rule = _DEFAULT_FUSED_RECURRENT_GATED_DELTA_RULE
    FusedRMSNormGated = _DEFAULT_FUSED_RMSNORM_GATED

if not hasattr(ALL_ATTENTION_FUNCTIONS, "get_interface"):
    def _get_attention_interface(name, default):
        try:
            return ALL_ATTENTION_FUNCTIONS[name]
        except Exception:
            return default

    ALL_ATTENTION_FUNCTIONS.get_interface = _get_attention_interface
...
def _normalize_torch_dtype(dtype_value):
    if not isinstance(dtype_value, str):
        return dtype_value
    normalized_name = dtype_value.strip().lower().replace("torch.", "")
...
class Qwen3_5GatedDeltaNet(nn.Module):
    def __init__(self, config: Qwen3_5Config, layer_idx: int):
        ...
        if FusedRMSNormGated is None:
            ...
            fused_norm_kwargs = {
                ...
                "dtype": _normalize_torch_dtype(config.dtype)
    ...
    def forward(
        ...
        if use_precomputed_states:
            ...
            conv_kernel = self.conv1d.weight.reshape(
                self.conv1d.weight.shape[0],
                self.conv1d.weight.shape[-1],
            )
            ...
            if mixed_qkv.dtype != hidden_states.dtype:
            ...
            conv_input = mixed_qkv if mixed_qkv.dtype == conv_dtype else mixed_qkv.to(conv_dtype)
            if self.causal_conv1d_fn is not None:
                ...
                mixed_qkv = F.silu(self.conv1d(conv_input)[:, :, :seq_len])
            if mixed_qkv.dtype != hidden_states.dtype:
                mixed_qkv = mixed_qkv.to(hidden_states.dtype)
...
class Qwen3_5Model(Qwen3_5PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # vision tower moved before text embedding to avoif flip / flop offloadling
...
class Qwen3_5ForConditionalGeneration(Qwen3_5PreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        effective_first_iteration = bool(is_first_iteration)
        if not effective_first_iteration:
            if past_key_values is None:
                effective_first_iteration = True
            else:
                get_seq_length = getattr(past_key_values, "get_seq_length", None)
                if callable(get_seq_length):
                    try:
                        effective_first_iteration = int(get_seq_length()) == 0
                    except Exception:
                        effective_first_iteration = False
        ...
        model_inputs = super().prepare_inputs_for_generation(
            ...
            video_grid_thw=video_grid_thw,
            use_cache=use_cache,
            is_first_iteration=effective_first_iteration,
            **kwargs,
        )

        if not effective_first_iteration and use_cache:
