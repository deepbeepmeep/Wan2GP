...
class Sam3Image(torch.nn.Module):
    ...
    def _prepare_prompt(
        ...
        del feat_tuple, img_feats, img_pos_embeds, vis_feat_sizes
        has_visual_prompt = visual_prompt_embed is not None and visual_prompt_embed.shape[0] > 0
        if encode_text:
            prompt_parts = [txt_feats, geo_feats]
            prompt_mask_parts = [txt_masks, geo_masks]
            if has_visual_prompt:
                prompt_parts.append(visual_prompt_embed)
                prompt_mask_parts.append(visual_prompt_mask)
            prompt = torch.cat(prompt_parts, dim=0)
            prompt_mask = torch.cat(prompt_mask_parts, dim=1)
        elif has_visual_prompt:
            prompt = torch.cat([geo_feats, visual_prompt_embed], dim=0)
            prompt_mask = torch.cat([geo_masks, visual_prompt_mask], dim=1)
        else:
            prompt, prompt_mask = geo_feats, geo_masks
        return prompt, prompt_mask, backbone_out

    def _encode_prompt(self, *args, **kwargs):
        return self._prepare_prompt(*args, **kwargs)
    ...
    def forward_grounding(
        ...
        **kwargs,
        ...
        with torch.profiler.record_function("SAM3Image._prepare_prompt"):
            prompt, prompt_mask, backbone_out = self._prepare_prompt(
