...
def apply_rotary_emb_inplace(
    ...
    if rope_type == LTXRopeType.INTERLEAVED:
        return apply_interleaved_rotary_emb_inplace(input_tensor, *freqs_cis)
    elif rope_type == LTXRopeType.SPLIT:
        return apply_split_rotary_emb_inplace(input_tensor, *freqs_cis)
    else:
        raise ValueError(f"Invalid rope type: {rope_type}")
...
def apply_interleaved_rotary_emb_inplace(
    ...
    x = input_tensor.view(*input_tensor.shape[:-1], -1, 2)
    cos = cos_freqs.view(*cos_freqs.shape[:-1], -1, 2)
    sin = sin_freqs.view(*sin_freqs.shape[:-1], -1, 2)

    x0 = x[..., 0]
    x1 = x[..., 1]
    x0_orig = x0.clone()
    x0.mul_(cos[..., 0]).addcmul_(x1, sin[..., 0], value=-1)
    x1.mul_(cos[..., 1]).addcmul_(x0_orig, sin[..., 1])
    return input_tensor
