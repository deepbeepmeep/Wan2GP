...
def _resolve_semantic_model_file(model_dir, use_fp16=False):
    fp16_path = os.path.join(model_dir, "model_fp16.safetensors")
    fp32_path = os.path.join(model_dir, "model.safetensors")
    if use_fp16 and os.path.isfile(fp16_path):
        return fp16_path
    ...
    raise FileNotFoundError(
        f"Wav2Vec2Bert checkpoint is missing in '{model_dir}'. "
        "Expected model.safetensors and/or model_fp16.safetensors."
    )


def build_semantic_model(path_='./models/tts/maskgct/ckpt/wav2vec2bert_stats.pt', model_dir="./w2v-bert-2.0", use_fp16=False):
    model_file = _resolve_semantic_model_file(model_dir, use_fp16=use_fp16)
    semantic_model = offload.fast_load_transformers_model(
        model_file,
        modelClass=Wav2Vec2BertModel,
        defaultConfigPath=os.path.join(model_dir, "config.json"),
        default_dtype=torch.float16 if use_fp16 else torch.float32,
        writable_tensors=False,
    )
    semantic_model.eval()
    stat_mean_var = torch.load(path_, map_location="cpu")
