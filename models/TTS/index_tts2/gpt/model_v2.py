...
try:
    from transformers.generation import GenerationMixin as HFGenerationMixin
except Exception:
    try:
        from transformers.generation.utils import GenerationMixin as HFGenerationMixin
    except Exception:
        try:
            from transformers.generation_utils import GenerationMixin as HFGenerationMixin
        except Exception:
...
@contextmanager
def _temporary_module_device(device_name):
...
class _TqdmTokenProgressCriteria(StoppingCriteria if StoppingCriteria is not None else object):
    def __init__(self, progress_bar, prompt_length):
    ...
    def __call__(self, input_ids, scores, **kwargs):
        if input_ids is None:
            return False
        generated = max(0, int(input_ids.shape[1]) - self.prompt_length)
        delta = generated - self.last_generated
        if delta > 0:
            self.progress_bar.update(delta)
            self.last_generated = generated
        return False

class _AbortRequestedCriteria(StoppingCriteria if StoppingCriteria is not None else object):
    def __init__(self, abort_checker):
        self.abort_checker = abort_checker

    def __call__(self, input_ids, scores, **kwargs):
        if self.abort_checker is None:
            return False
        try:
            return bool(self.abort_checker())
        except Exception:
            return False
...
class GPT2InferenceModel(GPT2PreTrainedModel, HFGenerationMixin):
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, **kwargs):
        ...
        if (
            ...
            and HFCache is not None
            and not isinstance(past_key_values, HFCache)
        ):
            past_key_values = DynamicCache.from_legacy_cache(past_key_values)

        has_past = past_key_values is not None
        if has_past and HFCache is not None and isinstance(past_key_values, HFCache):
    ...
    def forward(
        ...
        # Under MMGP offload, runtime compute dtype can differ from raw CPU param dtype.
        # Prefer an explicit runtime dtype when available.
        target_dtype = getattr(self.transformer, "_model_dtype", None)
        try:
            if target_dtype is None:
                emb_weight = getattr(self.embeddings, "weight", None)
                if emb_weight is not None:
                    target_dtype = emb_weight.dtype
            if target_dtype is None:
                blocks = getattr(self.transformer, "h", None)
                if blocks is not None and len(blocks) > 0:
                    ln_1 = getattr(blocks[0], "ln_1", None)
                    ln_weight = getattr(ln_1, "weight", None) if ln_1 is not None else None
                    if ln_weight is not None:
                        target_dtype = ln_weight.dtype
        except Exception:
            pass
        if target_dtype is None:
            transformer_param = next(self.transformer.parameters(), None)
            if transformer_param is not None:
                target_dtype = transformer_param.dtype
        if target_dtype is not None and emb.dtype != target_dtype:
...
def build_hf_gpt_transformer(layers, model_dim, heads, max_mel_seq_len, max_text_seq_len, checkpointing, build_dtype=None):
    """
    ...
    with _temporary_default_dtype(build_dtype):
        gpt = GPT2Model(gpt_config)
        mel_pos = LearnedPositionEmbeddings(max_mel_seq_len, model_dim)
        text_pos = LearnedPositionEmbeddings(max_text_seq_len, model_dim)
    # Override the built in positional embeddings
    del gpt.wpe
    gpt.wpe = functools.partial(null_position_embeddings, dim=model_dim)
    # Built-in token embeddings are unused.
    del gpt.wte
    return gpt, mel_pos, text_pos, None, None
...
class UnifiedVoice(nn.Module):
    @staticmethod
    def _materialize_attention_buffers_if_meta(gpt_module):
        if gpt_module is None:
            return

        def _fix_one_attention(attn_module):
            if attn_module is None:
                return
            bias = getattr(attn_module, "bias", None)
            if torch.is_tensor(bias) and bias.device.type == "meta":
                q_len = int(bias.shape[-1]) if bias.ndim >= 2 else 0
                if q_len > 0:
                    causal = torch.tril(torch.ones((q_len, q_len), dtype=torch.bool, device="cpu")).view(1, 1, q_len, q_len)
                    attn_module.bias = causal
            masked_bias = getattr(attn_module, "masked_bias", None)
            if torch.is_tensor(masked_bias) and masked_bias.device.type == "meta":
                attn_module.masked_bias = torch.tensor(-1e4, device="cpu", dtype=torch.float32)

        blocks = getattr(gpt_module, "h", None)
        if blocks is None:
            return
        for block in blocks:
            _fix_one_attention(getattr(block, "attn", None))
            _fix_one_attention(getattr(block, "crossattention", None))
    ...
    def __init__(self, layers=8, model_dim=512, heads=8, max_text_tokens=120, max_mel_tokens=250, max_conditioning_inputs=1,
        ...
        build_fp16 = bool(gpt_build_fp16 and _env_enabled("INDEXTTS2_GPT_BUILD_FP16", default=True))
        build_meta = bool(gpt_build_meta and _env_enabled("INDEXTTS2_GPT_BUILD_META", default=True))
        build_dtype = torch.float16 if build_fp16 else None
        build_device = "meta" if build_meta else None
        with _temporary_module_device(build_device):
            with _temporary_default_dtype(build_dtype):
                ...
                self._materialize_attention_buffers_if_meta(self.gpt)
    ...
    def _install_layernorm_input_dtype_guard(module):
        if module is None or getattr(module, "_ln_input_dtype_guard", False):
            return

        def _guard(ln_module, args):
            if args is None or len(args) == 0:
                return None
            hidden_states = args[0]
            if not torch.is_tensor(hidden_states) or not torch.is_floating_point(hidden_states):
                return None
            weight = getattr(ln_module, "weight", None)
            if weight is None or hidden_states.dtype == weight.dtype:
                return None
            return (hidden_states.to(dtype=weight.dtype),) + tuple(args[1:])

        for submodule in module.modules():
            if isinstance(submodule, nn.LayerNorm):
                submodule.register_forward_pre_hook(_guard)
        module._ln_input_dtype_guard = True
    ...
    def post_init_gpt2_config(self, use_deepspeed=False, kv_cache=False, half=False):
        ...
        if self.use_accel and torch.cuda.is_available():
            ...
            if half:
                ...
                accel_gpt = accel_gpt.float()
            accel_gpt.eval()
            self.gpt = accel_gpt

            lm_head_with_norm = nn.Sequential(copy.deepcopy(self.final_norm), copy.deepcopy(self.mel_head))
            if half:
                lm_head_with_norm = lm_head_with_norm.half()
            else:
                lm_head_with_norm = lm_head_with_norm.float()
            lm_head_with_norm.eval()
            self.gpt.lm_head_with_norm = lm_head_with_norm
            self.accel_engine = AccelInferenceEngine(
                model=self.gpt,
                lm_head=self.gpt.lm_head_with_norm,
        ...
        runtime_dtype = torch.float16 if half else torch.float32
        inference_param = next(self.inference_model.parameters(), None)
        if inference_param is not None and inference_param.dtype != runtime_dtype:
            self.inference_model = self.inference_model.to(dtype=runtime_dtype)
        self.inference_model.transformer._model_dtype = runtime_dtype
        if not self.use_accel:
            self._install_layernorm_input_dtype_guard(self.inference_model.transformer)
    ...
    def release_runtime_generation_cache(self):
        if getattr(self, "inference_model", None) is not None:
            self.inference_model.store_mel_emb(None)
        if getattr(self, "accel_engine", None) is not None:
            self.accel_engine.release_runtime_cache()
    ...
    def inference_speech(self, speech_condition, text_inputs, emo_speech_condition=None, cond_lengths=None, emo_cond_lengths=None, emo_vec=None, use_speed=False, input_tokens=None, num_return_sequences=1,
        ...
        target_device = speech_conditioning_latent.device
        if text_inputs.device != target_device:
            text_inputs = text_inputs.to(target_device)
        tmp = torch.zeros(text_inputs.size(0), device=target_device)
        ...
        cg_max_total_tokens = hf_generate_kwargs.pop("cg_max_total_tokens", None)
        if cg_max_total_tokens is None:
            prompt_budget = int(self.cond_num + self.max_text_tokens + 3)
            gen_budget = int(self.max_mel_tokens if max_generate_length is None else max_generate_length)
            cg_max_total_tokens = prompt_budget + max(1, gen_budget)
        ...
        try:
            ...
            if self.accel_engine is not None and num_return_sequences == 1:
                output = self.accel_engine.generate(
                    ...
                    cg_max_total_tokens=int(cg_max_total_tokens),
                    stop_checker=abort_checker,
                )
            else:
                token_progress = tqdm(total=int(max(1, max_length - trunc_index)), desc="transformer_tokens", unit="tok", leave=True)
                stopping_criteria = hf_generate_kwargs.pop("stopping_criteria", None)
                if StoppingCriteriaList is not None:
                    ...
                    if StoppingCriteria is not None:
                        stopping_criteria.append(_TqdmTokenProgressCriteria(token_progress, trunc_index))
                        if abort_checker is not None:
                            stopping_criteria.append(_AbortRequestedCriteria(abort_checker))
                try:
