...
if os.name == 'nt':
    ...
    class IShellItemImageFactory(IUnknown):
        ...
        _methods_ = [
            COMMETHOD([], HRESULT, 'GetImage',
                      ...
                      (['out', 'retval'], ctypes.POINTER(wintypes.HBITMAP), 'phbm'))
        ]
    ...
    SHCreateItemFromParsingName.argtypes = [
        ...
        ctypes.c_void_p,
        ctypes.POINTER(GUID),
        ctypes.POINTER(ctypes.POINTER(IShellItemImageFactory))
    ]
    ...
    def get_thumbnail_as_base64(file_path, size=256):
        ...
        try:
            hr = SHCreateItemFromParsingName(
                file_path, None, IShellItemImageFactory._iid_, ctypes.byref(factory_ptr)
            )
            if hr != 0:
            ...
            if hbitmap_handle:
                try:
                    bitmap_info = BITMAP()
                    if GetObjectW(hbitmap_handle, ctypes.sizeof(bitmap_info), ctypes.byref(bitmap_info)) == 0:
                        return None, file_path

                    bmp = Image.frombuffer(
                        'RGB', (bitmap_info.bmWidth, bitmap_info.bmHeight),
                        ctypes.string_at(bitmap_info.bmBits, bitmap_info.bmWidthBytes * bitmap_info.bmHeight),
                        'raw', 'BGRX', bitmap_info.bmWidthBytes, -1
                    )
