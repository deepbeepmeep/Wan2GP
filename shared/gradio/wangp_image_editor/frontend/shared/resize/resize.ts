...
export class ResizeTool implements Tool {
	...
	private bound_pointer_move: ((event: FederatedPointerEvent) => void) | null = null;
	private bound_pointer_up: (() => void) | null = null;
	...
	async setup(
		...
		this.unsubscribers.push(context.dimensions.subscribe((dimensions) => {
		...
		this.unsubscribers.push(context.position.subscribe((position) => {
		...
		this.unsubscribers.push(context.scale.subscribe((scale) => {
			this.scale = scale;
	...
	cleanup(): void {
		if (this.resize_ui_container) {
			...
			this.resize_ui_container.destroy({ children: true });
			this.resize_ui_container = null;
		}
	...
	private setup_event_listeners(): void {
		...
		this.bound_pointer_move = this.handle_pointer_move.bind(this);
		this.bound_pointer_up = this.handle_pointer_up.bind(this);
		stage.on("pointermove", this.bound_pointer_move);
		stage.on("pointerup", this.bound_pointer_up);
		stage.on("pointerupoutside", this.bound_pointer_up);
	...
	private cleanup_event_listeners(): void {
		...
		if (this.bound_pointer_move) {
			stage.off("pointermove", this.bound_pointer_move);
			this.bound_pointer_move = null;
		}
		if (this.bound_pointer_up) {
			stage.off("pointerup", this.bound_pointer_up);
			stage.off("pointerupoutside", this.bound_pointer_up);
			this.bound_pointer_up = null;
		}
