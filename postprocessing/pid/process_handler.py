...
from postprocessing.pid.runtime import PID_FLUX2_POST_UPSAMPLING_VALUE, PID_FLUX_POST_UPSAMPLING_VALUE, is_pid_post_upsampling
...
class PiDProcessHandler:
    ...
    def _process_target(self, process_settings: dict | None) -> str:
        ...
        return PID_FLUX2_POST_UPSAMPLING_VALUE if target == PID_FLUX2_POST_UPSAMPLING_VALUE else PID_FLUX_POST_UPSAMPLING_VALUE

    def normalize_target_control(self, value: str | None) -> str:
        value = str(value or "").strip().lower()
        return value if is_pid_post_upsampling(value) else self.default_target_control
    ...
    def normalize_target_control_for_process(self, value: str | None, process_settings: dict) -> str:
        value = str(value or "").strip().lower()
        return value if is_pid_post_upsampling(value) else self._process_target(process_settings)
