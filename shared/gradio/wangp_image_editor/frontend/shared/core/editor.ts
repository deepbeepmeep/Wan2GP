...
async function texture_from_url(url: string): Promise<Texture> {
	const blob = await fetch_image_blob(url);
	if (blob) return Texture.from(await createImageBitmap(blob));
...
export class LayerManager {
	...
	private destroy_background_layer(destroy_image_textures = true): void {
		if (!this.background_layer) return;
		const blank_texture = (this.background_layer.children[0] as Sprite | undefined)
			?.texture;
		for (const child of this.background_layer.children) {
			if (!(child instanceof Sprite)) continue;
			if (
				child.texture === blank_texture ||
				child.texture instanceof RenderTexture ||
				destroy_image_textures
			) {
				child.texture.destroy(true);
			}
		}
		this.background_layer.destroy({ children: true });
		this.background_layer = null;
	}

	destroy(): void {
		this.destroy_background_layer(true);
		for (const layer of [...this.layers]) {
			this.delete_layer(layer.id);
		}
		for (const texture of this.draw_textures.values()) {
		...
		this.layer_store.set({ active_layer: "", layers: [] });
	}
	...
	create_background_layer(
		width: number,
		height: number,
		destroy_image_textures = true
	): Container {
		if (this.background_layer) {
			this.destroy_background_layer(destroy_image_textures);
		}
	...
	async add_layer_from_url(
		...
		try {
			texture = await texture_from_url(url);
			if (!is_current()) {
				texture.destroy(true);
				return "";
			}

			layer = this.create_layer({
				width,
				height,
				layer_name: "Layer 1",
				user_created: true
	...
	delete_layer(id: string): void {
		...
		if (index > -1) {
			...
			if (draw_texture) {
				draw_texture.destroy(true);
				this.draw_textures.delete(this.layers[index].container);
			}
			this.layers[index].container.destroy({ children: true });
	...
	private _resize_background_layer(
		...
		const backgroundImage = oldBackgroundLayer.children.find(
			(child) =>
		...
		// Keep the image texture while replacing the layer's blank render texture.
		...
		if (backgroundImageState) {
			const newBgImage = new Sprite(backgroundImageState.texture);
			newBgImage.width = backgroundImageState.width;
			newBgImage.height = backgroundImageState.height;
			...
			if (scale) {
				...
				newBgImage.position.set(
					backgroundImageState.x + offsetX,
					backgroundImageState.y + offsetY
				);
			}
...
export interface ImageEditorContext {
	...
	get_source_image: () => Blob | File | null;
...
export class ImageEditor {
	...
	public image_container!: Container;
	public command_manager: CommandManager;
	public layer_manager!: LayerManager;
	public tools: Map<string, Tool> = new Map<string, Tool>();
	public current_tool!: ToolbarTool;
	public current_subtool!: Subtool;
	public target_element: HTMLElement;
	public width: number;
	public height: number;
	public wangp_editor_empty = true;
	public wangp_value_dirty = false;
	public wangp_source_image: Blob | File | null = null;
	public wangp_source_image_clean = false;
	public wangp_update_overlay: (() => void) | null = null;
	...
	private wangp_resize_observer: ResizeObserver | null = null;
	private wangp_ticker_stop_timer: number | null = null;
	private wangp_ticker_active_until = 0;
	private wangp_renderer_width = 0;
	private wangp_renderer_height = 0;
	private wangp_last_surface_width = 0;
	private wangp_last_surface_height = 0;
	private wangp_export_depth = 0;
	private wangp_release_after_export = false;
	private wangp_unsubscribers: (() => void)[] = [];
	private wangp_min_zoom_unsubscribe: (() => void) | null = null;
	...
	private get context(): ImageEditorContext {
		...
		return {
			...
			get_source_image: () => this.wangp_source_image,
	...
	private wangp_is_visible(): boolean {
		return (
			this.target_element.offsetParent !== null &&
			this.target_element.getClientRects().length > 0
		);
	}

	public wangp_request_render(): void {
		if (!this.app || !this.app.renderer) return;
		if (!this.wangp_is_visible()) {
			this.wangp_stop_ticker();
			return;
		}
		const app = this.app as any;
		if (app.wangp_render_scheduled) return;
		app.wangp_render_scheduled = true;
		requestAnimationFrame(() => {
			if (!this.app || !this.app.renderer) return;
			app.wangp_render_scheduled = false;
			this.app.render();
			this.wangp_stop_ticker();
		});
	}

	private wangp_stop_ticker(): void {
		if (!this.app) return;
		this.app.stop?.();
		this.app.ticker?.stop();
	}

	public wangp_keep_ticker_active(duration = 800): void {
		if (!this.app) return;
		this.wangp_ticker_active_until = performance.now() + duration;
		this.app.start?.();
		this.app.ticker?.start();
		if (this.wangp_ticker_stop_timer !== null) {
			window.clearTimeout(this.wangp_ticker_stop_timer);
		}
		this.wangp_ticker_stop_timer = window.setTimeout(() => {
			this.wangp_ticker_stop_timer = null;
			if (performance.now() >= this.wangp_ticker_active_until) {
				this.wangp_stop_ticker();
			}
		}, duration);
	}

	public wangp_set_empty(empty: boolean): void {
		this.wangp_editor_empty = empty;
		this.target_element.classList.toggle("wangp-empty-image-editor", empty);
		this.target_element.parentElement?.classList.toggle(
			"wangp-empty-image-editor",
			empty
		);
		if (empty) {
			...
			this.wangp_restore_surface_for_content();
			this.wangp_update_overlay?.();
			this.wangp_request_render();
		}
	}

	public wangp_release_surface(): void {
		if (!this.app?.renderer) return;
		if (this.wangp_export_depth > 0) {
	...
	private wangp_begin_export(): void {
		this.wangp_export_depth += 1;
		this.wangp_restore_surface_for_content();
	}

	private wangp_end_export(): void {
		this.wangp_export_depth -= 1;
		if (this.wangp_export_depth === 0) {
			this.wangp_release_after_export = false;
			if (this.wangp_editor_empty || !this.wangp_is_visible()) {
				this.wangp_release_surface();
			}
		}
	}

	private wangp_resize_renderer(width: number, height: number): void {
		const next_width = Math.max(1, Math.round(width));
		const next_height = Math.max(1, Math.round(height));
		if (
			this.wangp_renderer_width === next_width &&
			this.wangp_renderer_height === next_height
		) {
			return;
		}
		this.app.renderer.resize(next_width, next_height);
		this.wangp_renderer_width = next_width;
		this.wangp_renderer_height = next_height;
	}

	private wangp_remember_surface_size(width: number, height: number): void {
		const next_width = Math.round(width);
		const next_height = Math.round(height);
		if (next_width > 1 && next_height > 1) {
			this.wangp_last_surface_width = next_width;
			this.wangp_last_surface_height = next_height;
		}
	}

	private wangp_restore_surface_for_content(): void {
		const rect = this.target_element.getBoundingClientRect();
		const width = rect.width > 0 ? rect.width : this.wangp_last_surface_width;
		const height = rect.height > 0 ? rect.height : this.wangp_last_surface_height;
		if (width > 0 && height > 0) {
			this.resize_canvas(width, height);
		}
	}

	public wangp_resize_to_element(): void {
		const rect = this.target_element.getBoundingClientRect();
		if (rect.width > 0 && rect.height > 0) {
			this.resize_canvas(rect.width, rect.height);
		}
	}

	public wangp_reset_crop_tool(): void {
		const crop_tool = this.tools.get("crop") as CropTool;
		(crop_tool as any)?.wangp_reset_interaction?.();
	}

	async init(): Promise<void> {
		const container_box = this.target_element.getBoundingClientRect();
		this.wangp_resize_observer = new ResizeObserver((entries) => {
		...
		this.wangp_resize_observer.observe(this.target_element);
		...
		try {
			await this.app.init({
				...
				backgroundAlpha: 0,
				resolution: this.dark ? 1 : window.devicePixelRatio,
				autoDensity: true,
				antialias: !this.dark,
				preference: "webgl",
		...
		(this.app as any).wangp_request_render = () => this.wangp_request_render();
		this.wangp_renderer_width = Math.max(1, Math.round(container_box.width));
		this.wangp_renderer_height = Math.max(1, Math.round(container_box.height));
		this.wangp_remember_surface_size(container_box.width || this.width, container_box.height || this.height);
		...
		this.wangp_unsubscribers.push(this.dimensions.subscribe((dimensions) => {
		...
		this.wangp_unsubscribers.push(this.scale.subscribe((scale) => {
			this.scale_value = scale;
			this.wangp_update_overlay?.();
		}));

		this.wangp_unsubscribers.push(this.position.subscribe((position) => {
		...
		this.wangp_update_overlay();
		((window as any).__wangpImageEditors ||= []).push(this);
		this.ready_resolve();
	}
	...
	resize_canvas(width: number, height: number): void {
		if (!this.app?.renderer || width <= 0 || height <= 0) return;
		this.wangp_remember_surface_size(width, height);
		if (this.wangp_editor_empty) {
			this.wangp_stop_ticker();
			return;
		}
		const restore_released_surface =
			this.wangp_renderer_width === 1 &&
			this.wangp_renderer_height === 1 &&
			this.wangp_is_visible();
		this.wangp_resize_renderer(width, height);
		...
		this.wangp_update_overlay?.();
		if (restore_released_surface) this.reset();
	}

	reset(): void {
		const zoom = this.tools.get("zoom");
		if (zoom) {
			(zoom as ZoomTool).set_zoom("fit");
		}
	}
	...
	async add_image({
		...
		this.wangp_set_empty(false);
		this.wangp_source_image = image instanceof Blob ? image : null;
		this.wangp_source_image_clean = !!this.wangp_source_image;
		...
		this.wangp_update_overlay?.();
		if (!this.wangp_is_visible()) this.wangp_release_surface();
	}
	...
	async add_image_from_url(
		...
		this.wangp_set_empty(false);
		const image_tool = this.tools.get("image") as ImageTool;
		let source: Blob | Texture | null = await fetch_image_blob(url);
		if (source) {
			this.wangp_source_image = source;
			this.wangp_source_image_clean = true;
		} else {
			source = await Assets.load(url);
			this.wangp_source_image = null;
			this.wangp_source_image_clean = false;
		}
		if (!is_current()) {
			if (source instanceof Texture) source.destroy(true);
			return;
		}
		await image_tool.add_image({
			image: source,
	...
	async reset_canvas(notify = true): Promise<void> {
		...
		if (notify) {
			...
			this.wangp_value_dirty = false;
			this.wangp_update_overlay?.();
		}
		this.wangp_set_empty(true);
	}
	...
	async add_layers_from_url(
		...
		if (layers.length > 0) {
			this.layer_manager.set_active_layer(layers[0].id);
		}
	...
	destroy(): void {
		...
		this.wangp_resize_observer?.disconnect();
		this.wangp_resize_observer = null;
		this.wangp_min_zoom_unsubscribe?.();
		this.wangp_min_zoom_unsubscribe = null;
		for (const unsubscribe of this.wangp_unsubscribers.splice(0)) {
			unsubscribe();
		}
		for (const tool of this.tools.values()) {
			tool.cleanup();
		}
		this.layer_manager?.destroy();
		this.wangp_update_overlay = null;
		this.app.destroy({ removeView: true }, { children: true });
	}

	private wangp_bind_min_zoom(): void {
		this.wangp_min_zoom_unsubscribe?.();
		const zoom = this.tools.get("zoom") as ZoomTool;
		this.wangp_min_zoom_unsubscribe = zoom
