...
from source.core.log.display_names import friendly_task_id
...
def _get_video_resolution(video_path: str | Path) -> Tuple[int, int] | None:
    ...
    try:
        ...
        if result.returncode != 0:
            orchestrator_logger.debug_anomaly("GET_RESOLUTION", f"ffprobe failed: {result.stderr}")
            return None
        width_str, height_str = result.stdout.strip().split(',')
        return (int(width_str), int(height_str))
    except (subprocess.SubprocessError, OSError, ValueError) as e:
        orchestrator_logger.debug_anomaly("GET_RESOLUTION", f"Error detecting resolution: {e}")
        return None
...
def _handle_join_clips_orchestrator_task(
    ...
    try:
        ...
        join_anchor_id = friendly_task_id(orchestrator_task_id_str, "join_clips_orchestrator")
        ...
        # First wake — emit the full planning anchor + SETUP card.
        print("")  # Visual breathing room before a new task anchor
        orchestrator_logger.essential(
            f"▸ Join [{join_anchor_id}] planning {len(clip_list)} clips",
            task_id=orchestrator_task_id_str,
            include_task_prefix=False,
        )
        orchestrator_logger.debug_block(
            "SETUP",
            {
                ...
                "segment_task_ids": len(segment_task_ids),
                "loop_first_clip": loop_first_clip,
        ...
        if enhance_prompt:
            ...
            try:
                ...
                orchestrator_logger.warning(f"JOIN_ORCHESTRATOR VLM enhancement failed, using base prompts: {vlm_error}", exc_info=True)
                vlm_enhanced_prompts = [None] * num_joins

        valid_vlm_prompts = sum(1 for p in vlm_enhanced_prompts if p is not None)
        orchestrator_logger.debug_block(
            "CLIP_CHAIN",
            {
                "join_count": num_joins,
                "frame_validation": "skipped" if skip_validation else "passed",
                "frame_counts": frame_counts,
                "resolution": join_settings.get("resolution"),
                "enhance_prompt": enhance_prompt,
                "vlm_prompts": f"{valid_vlm_prompts}/{num_joins}",
                "pattern": "parallel" if orchestrator_payload.get("use_parallel_joins", True) else "chain",
            },
            task_id=orchestrator_task_id_str,
        )
        ...
        orchestrator_logger.essential(
            f"▸ Join [{join_anchor_id}] spawning {num_joins} join tasks",
            task_id=orchestrator_task_id_str,
            include_task_prefix=False,
        )
        ...
        if use_parallel:
            orchestrator_logger.essential(
                f"▸ Join [{join_anchor_id}] final stitch",
                task_id=orchestrator_task_id_str,
                include_task_prefix=False,
            )
        ...
        if success:
            orchestrator_logger.essential(
                f"▸ Join [{join_anchor_id}] awaiting {num_joins} joins",
                task_id=orchestrator_task_id_str,
                include_task_prefix=False,
            )
        ...
        orchestrator_logger.debug_anomaly("JOIN_ORCHESTRATOR", msg, task_id=orchestrator_task_id_str)
        return False, msg
